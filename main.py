        ...
        def process():
            ...
            try:
                ...
                if file.validate():
                    ...
                    # archive_path = os.path.join(ARCHIVE_DIR, file.loading_path)
                    # os.makedirs(os.path.dirname(archive_path), exist_ok=True)
                    # shutil.move(file_path, archive_path + '/' + file.file_name)
            except Exception as e:
                log_action("[ERROR]", f"Failed to process {file.file_name}: {e}")
                # unprocessed_path = os.path.join(UNPROCESSED_DIR, file.loading_path)
